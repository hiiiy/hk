...
public class HelloServletOld extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException {
		...
		//servletContext객체 얻어오기(jsp 에서 scope 객체중 application 객체를 의미함)
		ServletContext application=config.getServletContext();
		application.setAttribute("id", "id");//객체 저장
		//web.xml에 값을 정의해놓고 가져올 수도 있다.
		String driver=application.getInitParameter("driver");
		System.out.println("application 객체에 저장된 값:"+driver);
	}
	//service()를 구현함
	...
	//request 같은 객체는 새롭개 생성하지 않고,받은 객체를 바로 쓰거나, 전달해서 사용
...
//		out.print("<h2>서블릿기에서 받은 파라미터:"+param+"</h2>");
